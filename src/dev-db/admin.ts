document.addEventListener("DOMContentLoaded", async () => {
  ...
  const adminEditTabActions = document.querySelector<HTMLElement>(
    `[dev-target=admin-edit-tab-actions]`
  )!;
  const adminEditTabApprove = adminEditTabActions.querySelector<HTMLElement>(
    `[dev-target=admin-edit-approve]`
  )!;
  const adminEditTabMoveToLive = adminEditTabActions.querySelector<HTMLElement>(
    `[dev-target=edit-tab-move-to-live]`
  )!;
  const adminEditTabReject = adminEditTabActions.querySelector<HTMLElement>(
    `[dev-target=admin-edit-reject]`
  )!;
  const adminEditTabDeleteReject =
    adminEditTabActions.querySelector<HTMLElement>(
      `[dev-target=admin-edit-delete-rejected-insight]`
    )!;
  ...
  function displayRowsOnTable(
    ...
    data.items.forEach((insight) => {
      ...
      const sourceListWrap = row.querySelector<HTMLElement>(
        "[dev-target=source-list]"
      );
      const sourceListItem = row.querySelector<HTMLElement>(
        "[dev-target=source-list-item]"
      );
      ...
      name?.addEventListener("click", () => {
        localStorage.setItem(
          ...
          JSON.stringify(insight["insight-detail"])
        );
      });
  ...
  function updateAdminEditTabActionsStates(insight: Insight) {
    currentInsight = insight;
    adminEditTabActions.setAttribute(
      "dev-display",
      editTableNameValue === "editor_insights" ? "flex" : "none"
    );
  ...
  async function adminAction(
    action: "approve" | "reject" | "delete" | "move-to-live",
    id: number
  ) {
    const res = await fetch(
