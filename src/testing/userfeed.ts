...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const lsInsights = localStorage.getItem("pft_insights");
  const lsFollowingInsights = localStorage.getItem("pft_insightsFollowing");
  const lsFavouriteInsights = localStorage.getItem("pft_insightsFavourite");
  ...
  async function getInsights(
    ...
    try {
      ...
      if (
        ...
        localStorage.setItem("pft_insights", JSON.stringify(insights));
      }
      if (
        ...
        localStorage.setItem("pft_insightsFollowing", JSON.stringify(insights));
      }
      if (
        ...
        localStorage.setItem("pft_insightsFavourite", JSON.stringify(insights));
      }
      userFollowingAndFavourite &&
        ...
        .forEach((item) => {
          ...
          if (inputType === "company_id") {
            newFollowingItem
              ...
              ?.setAttribute("href", "/pft/company_pft/" + item.slug);
          }
          if (inputType === "event_id") {
            newFollowingItem
              .querySelector("[dev-target=link]")
              ?.setAttribute("href", "/pft/event_pft/" + item.slug);
          }
          if (inputType === "people_id") {
            newFollowingItem
              .querySelector("[dev-target=link]")
              ?.setAttribute("href", "/pft/person_pft/" + item.slug);
          }
          if (inputType === "technology_category_id") {
            newFollowingItem
              .querySelector("[dev-target=link]")
              ?.setAttribute("href", "/pft/technology_pft/" + item.slug);
          }
...
interface UserFollowingAndFavourite {
  user_following: {
    id: number;
    created_at: number;
    xano_user_id: number;
