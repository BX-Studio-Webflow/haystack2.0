...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const searchInput = qs("[dev-global-search]");
    const searchResultWrapper = qs("[dev-global-search-result-wrapper]");
    const companySearchItem = qs("[dev-template=company-search-item]");
    const insightSearchItem = qs("[dev-template=insight-search-item]");
    const personSearchItem = qs("[dev-template=person-search-item]");
    const eventSearchItem = qs("[dev-template=event-search-item]");
    ...
    async function searchCompany(searchQuery) {
        try {
            ...
            companies.forEach((company) => {
                const companyItem = companySearchItem.cloneNode(true);
                ...
                companyItem.href = "/pft/company_pft/" + company.slug;
                ...
                about?.classList[about.textContent.toLowerCase().includes(searchQuery.toLowerCase())
                    ? "remove"
                    : "add"]("hide");
                smallDesc?.classList[smallDesc.textContent
                    .toLowerCase()
    ...
    async function searchInsight(searchQuery) {
        try {
            ...
            insights.forEach((insight) => {
                ...
                insightItem.href = "/pft/insight_pft/" + insight.slug;
    ...
    async function searchPerson(searchQuery) {
        try {
            const res = await xano_global_search.get("/person", {
                search_query: searchQuery,
            });
            const people = res.getBody();
            ...
            people.forEach((person) => {
                const personItem = personSearchItem.cloneNode(true);
                ...
                personItem.href = "/pft/person_pft/" + person.slug;
                ...
                title?.classList[title.textContent.toLowerCase().includes(searchQuery.toLowerCase())
                    ? "remove"
                    : "add"]("hide");
                bio?.classList[bio.textContent.toLowerCase().includes(searchQuery.toLowerCase())
                    ? "remove"
                    : "add"]("hide");
    ...
    async function searchEvent(searchQuery) {
        try {
            ...
            events.forEach((event) => {
                const eventItem = eventSearchItem.cloneNode(true);
                ...
                eventItem.href = "/pft/event_pft/" + event.slug;
                ...
                city?.classList[city.textContent.toLowerCase().includes(searchQuery.toLowerCase())
                    ? "remove"
                    : "add"]("hide");
                venue?.classList[venue.textContent.toLowerCase().includes(searchQuery.toLowerCase())
                    ? "remove"
                    : "add"]("hide");
    ...
    function highlightSearchQuery(paragraph, searchQuery) {
